...
class AdminMenu extends CActiveRecord {
    ...
    public function attributeLabels() {
        return array(
            ...
            'parentid' => Yii::t('system','parent menu'),
            'modules' => Yii::t('system','modules'),
            'controller' => Yii::t('system','controller'),
            'action' => Yii::t('system','action'),
            'data' => Yii::t('system','parameters data'),
            'ico' => Yii::t('system','ico'),
            'listorder' => Yii::t('system','order'),
            'display' => Yii::t('system','display'),
        );
        //  $this->search()
    }
    /**
    ...
    public function beforeDelete() {
        ...
        if($count != 0)
            throw new CDbException(Yii::t('system','This menu have one or more submenus, please remove all submenus before delete.'));
        return true;    
    }
    //后台菜单
    public static function getTreeDATA($select = null,$cache = TRUE) {
        ...
        foreach($menus as $menu) {
            $menu['name']=Yii::t('menus',$menu['name']);
            $array[$menu['id']] = $menu;
        } 
        $menus = $array;
    ...
    public static function getSelectTree($empty = NULL, $pid = 0) {
        ...
        if ($empty !== NULL)
            return '<option value="">' . $empty . '</option>' . $tree->get_tree('0', $str);
        else
