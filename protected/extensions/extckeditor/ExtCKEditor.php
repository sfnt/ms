...
//advanced - пользователь сам определяет тулбар
    ...
    private $ckSkin='moono';
    private $theme='default';


    public function  __construct($owner=null) {
        parent::__construct($owner);
        if(isset(Yii::app()->params['upload_dir'])){
            $this->setFilePath(Yii::app()->basePath.'/../'.Yii::app()->params['upload_dir']);
            $this->setFileURL(Yii::app()->baseUrl.'/'.Yii::app()->params['upload_dir']);
        }
    ...
    public function setFilePath($value){
        if (!is_string($value))
            throw new CException(Yii::t(__CLASS__, 'FilePath must be an string'));
        $this->filePath = $value;
    }
    ...
    public function setFileURL($value){
        if (!is_string($value))
            throw new CException(Yii::t(__CLASS__, 'FileURL must be an URL'));
        $this->fileURL = $value;
    }
    ...
    // to make the content look like if it were in your target page
        ...
        if ($this->filePath&&$this->fileURL) {
            $fckDir = Yii::app()->getAssetManager()->publish(dirname(__FILE__).DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'kcfinder',true);
            $options['filebrowserBrowseUrl']=$fckDir.'/browse.php?type=files';
            $options['filebrowserImageBrowseUrl']=$fckDir.'/browse.php?type=images';
            $options['filebrowserFlashBrowseUrl']=$fckDir.'/browse.php?type=flash';
            $options['filebrowserUploadUrl']=$fckDir.'/upload.php?type=files';
            $options['filebrowserImageUploadUrl']=$fckDir.'/upload.php?type=images';
            $options['filebrowserFlashUploadUrl']=$fckDir.'/upload.php?type=flash';
            $session=new CHttpSession;
            $session->open();
            $session['KCFINDER'] = array(
                    'disabled'=>false,
                    'uploadURL'=>$this->fileURL,
                    'uploadDir'=>realpath($this->filePath).'/',
                    'maxImageWidth' => isset(Yii::app()->params['upload_img_max_width'])?Yii::app()->params['upload_img_max_width']:0,
                    'maxImageHeight' => isset(Yii::app()->params['upload_img_max_height'])?Yii::app()->params['upload_img_max_height']:0,
                    );
        }
    ...
    public function run(){
        ...
        $assets = Yii::app()->getAssetManager()->publish($baseDir.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'ckeditor',true);
